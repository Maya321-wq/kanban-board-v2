...
const ListColumn = memo(function ListColumn({ list }) {
  ...
  const cards = state.cards.filter(card => card.listId === list.id);
  const cardIds = cards.map(card => `card-${card.id}-${list.id}`);
  ...
  const handleAddCard = useCallback(() => {
    ...
    if (title && title.trim()) {
      addCard({ listId: list.id, title: title.trim() });
    }
  }, [addCard, list.id]);
  ...
  const handleRename = useCallback(() => {
    ...
    if (title && title.trim()) {
      renameList(list.id, title.trim());
    }
  }, [renameList, list.id, list.title]);

  const handleArchive = useCallback(() => {
    if (window.confirm(`Archive list "${list.title}" and all its cards?`)) {
      archiveList(list.id);
    }
  }, [archiveList, list.id, list.title]);

  return (
    <div className="w-72 flex-shrink-0 bg-white/95 backdrop-blur-sm rounded-xl shadow-xl flex flex-col max-h-[calc(100vh-12rem)] border border-gray-200">
      {/* List Header */}
      <div className="flex justify-between items-center p-4 border-b border-gray-200 bg-gradient-to-r from-gray-50 to-white rounded-t-xl">
        <h3 className="font-bold text-base text-gray-800" aria-label={`List: ${list.title}, ${cards.length} cards`}>
        ...
        <div className="flex gap-2">
          <button
            ...
            className="text-gray-400 hover:text-blue-600 p-1.5 rounded-lg hover:bg-blue-50 transition-all duration-200"
            title="Rename list"
          >
            ✏️
          </button>
      ...
      <div
        ref={setNodeRef}
        className="flex-1 overflow-y-auto p-3 space-y-3 bg-gradient-to-b from-gray-50/50 to-transparent"
      >
      ...
      <button
        ...
        aria-label={`Add card to list ${list.title}`}
      >
